<section id="slider">
    <div class="flexslider">
        <ul class="slides">
            <!-- Slide -->
            <li>
                <h1>CARNIA DESIGN STUDIO<br>
                    <p class="lead">
                        Ние дизајнираме и развиваме прекрасни веб страни и веб апликации. <br>
                        Нашите методи се современи; нашите дела - препознатливи.
                    </p>
                </h1>
                <img src="/assets/img/bck.jpg" alt="">
            </li>
        </ul>
    </div><!-- /flexslider -->
</section><!-- #slider -->

<div class="container">
    <div class="row">
        <div class="col-md-6">
            <div class="rightline">
            <h1>Нашата Мисија</h1>
            <p>Maximum mission is to deliver the highest quality web design and development based on a fresh approach to customer service and value for money.</p>
            <p>We strive to offer a full suite of web based services inhouse and keep abreast of the latest technologies to ensure our customers are kept up to date on the latest internet based trends.</p>
            </div><!-- /rightline -->
        </div><!-- /col-md-6 -->
        <div class="col-md-6">
            <h1>Нашата Пасија</h1>
            <p>Maximum are passionate about what we do, we stand by our work and your investment by ensuring that only our best, most qualified developers maintain it.</p>
            <p>It is a common occurrence for a new site to be built and then quickly handed over to a support team and forgotten about. We are an exception, our customers are our most valuable advertisements, we stand by them!</p>
        </div><!-- /col-md-6 -->
    </div><!-- /row -->
    <hr>
</div>
<div class="container">
    <div class="row">
        <div class="middle">
        <h2>Наша Експертиза</h2>
        <br>
            <div class="col-md-3">
                <img src="/assets/img/service01.png" alt="">
                <br>
                <br>
                <p><strong>Веб Дизајн</strong>: Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took...</p>
            </div><!-- /col-md-3 -->
            <div class="col-md-3">
                <img src="/assets/img/service02.png" alt="">
                <br>
                <br>
                <p><strong>Интеграција</strong>: Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took...</p>
            </div><!-- /col-md-3 -->
            <div class="col-md-3">
                <img src="/assets/img/service03.png" alt="">
                <br>
                <br>
                <p><strong>Маркетинг</strong>: Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took...</p>
            </div><!-- /col-md-3 -->
            <div class="col-md-3">
                <img src="/assets/img/service04.png" alt="">
                <br>
                <br>
                <p><strong>Развој</strong>: Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took...</p>
            </div><!-- /col-md-3 -->
        </div><!-- /middle -->
    </div><!-- /row -->
    <br>
    <br>
</div>